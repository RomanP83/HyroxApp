...
-- ─────────────────────────────────────────────────────────────────────
-- supabase/migrations/0027_double_days.sql
-- ─────────────────────────────────────────────────────────────────────
-- ============================================================================
...
-- ─────────────────────────────────────────────────────────────────────
-- supabase/migrations/0028_plan_start_date.sql
-- ─────────────────────────────────────────────────────────────────────
-- ============================================================================
-- 0028 — when week 1 begins.
--
-- Two things were wrong, and they looked like one.
--
-- First, nothing ever advanced plan_weeks.status. persist_plan marked week 1
-- 'current' and no code in the app ever moved it, so seven readers — the plan
-- page, the nightly check-in, the weekly review, Strava auto-logging, the
-- benchmark association, the settings volume comparison and the macro cron —
-- were pinned to week 1 for the life of the plan. That is fixed by DERIVING
-- the current week from a date rather than storing it: a derived value cannot
-- go stale and a rebase has nothing to fix up. plan_weeks.status stays as the
-- log it always was ('completed', 'rebased').
--
-- Second, the date it was derived from did not exist. The grid was anchored on
-- generated_at — the moment the athlete pressed the button — so a plan built
-- on a Saturday gave week 1 two days. starts_on is that anchor made explicit
-- and movable: always a Monday, chosen by the athlete, defaulting to the next
-- one.
--
-- Existing plans keep exactly the grid they had: the Monday of the week they
-- were generated in.
-- ============================================================================
...
update plans
set starts_on = (generated_at at time zone 'UTC')::date
  - ((extract(isodow from (generated_at at time zone 'UTC'))::int - 1) || ' days')::interval
where starts_on is null;

alter table plans
  alter column starts_on set not null,
  alter column starts_on set default (
    current_date - ((extract(isodow from current_date)::int - 1) || ' days')::interval
  )::date;

-- persist_plan carries it through (replaces the 0014 definition).
create or replace function persist_plan(p jsonb)
returns uuid
language plpgsql
security definer
...
begin
  ...
  insert into plans (profile_id, race_id, race_date, starts_on, status, total_weeks, engine_version, stripe_payment_id)
  values (
    v_profile,
    nullif(p->>'race_id', '')::uuid,
    (p->>'race_date')::date,
...
-- ─────────────────────────────────────────────────────────────────────
-- supabase/migrations/0029_plan_kind.sql
-- ─────────────────────────────────────────────────────────────────────
-- ============================================================================
...
-- ─────────────────────────────────────────────────────────────────────
-- supabase/migrations/0030_race_results.sql
-- ─────────────────────────────────────────────────────────────────────
-- ============================================================================
...
-- ──────────────────────────────────────────────────────────
-- supabase/migrations/0031_measured_station_seconds.sql
-- ──────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────
-- supabase/migrations/0032_goal_race_time.sql
-- ──────────────────────────────────────────────────────────
-- ============================================================================
...
-- ──────────────────────────────────────────────────────────
-- supabase/migrations/0033_station_substitutions.sql
-- ──────────────────────────────────────────────────────────
-- ============================================================================
